    ...
    public static class ServerDebugExtensions
    {
        public static Task<Result<ServerInfo>> ScreenDump(this Task<Result<ServerInfo>> result)
        {
            ...
            foreach (var queue in results.Queues)
            {
                ...
                Console.WriteLine($"\tReductions: {queue.ReductionDetails?.Rate} (rate)");
                Console.WriteLine($"\tMessages Unacknowledged: {queue.UnacknowledgedMessageDetails?.Rate} (rate)");
                Console.WriteLine($"\tMessages Ready: {queue.ReadyMessageDetails?.Rate} (rate)");
                Console.WriteLine($"\tMessages: {queue.MessageDetails?.Rate} (rate)");
