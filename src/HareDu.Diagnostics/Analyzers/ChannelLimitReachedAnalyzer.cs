        ...
        IDiagnosticAnalyzer
    {
        ...
        public DiagnosticResult Execute<T>(T snapshot)
        {
            ...
            if (Convert.ToUInt64(data.Channels.Count) >= data.ChannelLimit)
            {
                ...
                result = new NegativeDiagnosticResult(data.Identifier, Identifier, ComponentType, analyzerData, knowledgeBaseArticle);
            }
            else
            {
                _knowledgeBaseProvider.TryGet(Identifier, DiagnosticStatus.Green, out knowledgeBaseArticle);
                result = new PositiveDiagnosticResult(data.Identifier, Identifier, ComponentType, analyzerData, knowledgeBaseArticle);
            }
